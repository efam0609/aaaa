...
public class frmPrincipal extends javax.swing.JFrame {
    ...
    {               
        ...
        try
        {
            ...
            con.getConnection().prepareStatement("SELECT * FROM usuarios WHERE  IdUsuario='"+cUser+"'");
    ...
    void destinoBackup()
    {
        ...
        if (respuesta == JFileChooser.APPROVE_OPTION)
        {
            ...
            /*************************              
            *ahora guardamos esa ruta en la base de datos. pero antes debemos verificar si la tabla esta vacia
            * si lo esta hacemos un insert de lo contrario un update           
            */
            try
            {                
                con = new bdConexion();
                ...
                if(resultado.getRow()<=0)//significa que esta vacia la tabla
                {
                    try
                    {
                        ...
                        con.getConnection().prepareStatement("insert into " + " rutabackup(rutaBD)" + "values (?)");
                ...
                else // significa que contiene informacion, asi que actualizamos
                {
                    try
                    {
                        con = new bdConexion();
                        PreparedStatement pstm=(PreparedStatement)
                        con.getConnection().prepareStatement("UPDATE rutabackup SET rutaBD='"+rutaCorregida+"'");
                        pstm.executeUpdate();
                        pstm.close();
                    }
                    catch(SQLException e)
    ...
    void GenerarBackupAutomatico() throws SQLException{
        ...
        { 
            ...
            //con la siguente consulta obtnemos la ruta almacenada en la bd para guardar ahi la copia de seguridad
            con.getConnection().prepareStatement("SELECT * FROM rutabackup");
